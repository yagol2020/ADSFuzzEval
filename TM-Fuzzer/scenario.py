...
class Scenario:
    ...
    def run_test(self, exec_state):
        ...
        self.state.r_gid = self.generation_id
        self.state.r_sid = self.scenario_id
        param = (
            self.conf,
            self.state,
            exec_state,
        ...
        print("start os.system simulate.py")
        os.system("python simulate.py")
        try:
            ret, self.npc_list, self.state = dill.load(open("ret.pick", "rb")), dill.load(
                open("npc.pick", "rb")), dill.load(open("state.pick", "rb"))
        except:
            print("dill load error")
            sys.exit(-99)
        if ret == -1:
            return -1
        ...
        shutil.copyfile("param.pick", os.path.join(self.conf.queue_dir, log_filename.replace(".json", ".param.pick")))
    ...
    def save_video(self, error, log_filename):
        try:
            if error:
                ...
                shutil.copyfile(
                    f"/tmp/fuzzerdata-tmfuzz/front.mp4",
                    os.path.join(
                        self.conf.cam_dir,
                ...
                shutil.copyfile(
                    f"/tmp/fuzzerdata-tmfuzz/top.mp4",
                    os.path.join(
                        self.conf.cam_dir,
