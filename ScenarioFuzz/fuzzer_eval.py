...
def set_args():
    ...
    argparser.add_argument("--cache-dir", default="/tmp/fuzzerdata-scenariofuzz", type=str,
    ...
    argparser.add_argument("-s", "--seed-dir", default=None, type=str,
                           help="Seed directory")
    argparser.add_argument("--seed-smy-dir", default=None, type=str,
                           help="Seed directory")
    ...
    while True:
        ...
        while cycle_cnt < conf.max_cycles:
            ...
            if successor_scenario_value is not None and not args.single_stage:
                ...
                samples = sampler_class.sample(method=sampler.SamplingMethod.RANDOM_NEIGHBORS, num_samples=NUM_SAMPLE,
                                               reference_data=test_scenario_value)
            else:
            ...
            with torch.no_grad():
                ...
                for batch in test_loader:
                    ...
                    out = eval_model(batch.x, batch.edge_attr, batch.weather_attr, batch.edge_index, batch.batch,
                                     batch.edge_batch)
            ...
            while round_cnt < conf.max_mutations:  # mutation rounds
                ...
                for ov_attri, ov_s_w in zip(ov_list, ov_wp_sp_list):
                    ...
                    if v_type == C.VEHICLE:
                        ...
                        ret = test_scenario_m.add_actor(v_type, nav_type, v_s, v_w, walker_speed, w_name,
                                                        dp_time=dp_time)

                    if conf.debug:
                        if v_type != C.NULL:
                            print("[debug] successfully added {} {}".format(C.NAVTYPE_NAMES[nav_type],
                                                                            C.ACTOR_NAMES[v_type]))

                # STEP 3-2: PUDDLE PROFILE GENERATION
                puddle_list = sample['Puddle']
                for puddle_inf in puddle_list:
                    center_loc_x, center_loc_y, center_loc_z = scenario_dict['center_loc'][0], \
                    scenario_dict['center_loc'][1], scenario_dict['center_loc'][2]
                ...
                sc.save_json(os.path.join(conf.scenario_data, string), test_scenario_m.driving_quality_score,
                             test_scenario_m.event_dict, G_data_now,
                             sample_data_now,
