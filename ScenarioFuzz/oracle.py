...
class BaseOracle(object):
    ...
    def save_2_file(self, report_dir):
        path = os.path.join(
            report_dir, f"{self.oracle_name}.json"
        )
        try:
            json.dump(
                self.report, open(path, "w", encoding="utf-8"), cls=CustomEncoder, indent=4
            )
...
class CollisionOracle(BaseOracle):
    ...
    def on_carla_collision_sensor(self, event):
        r = {
            ...
            "detected_time": time.time(), "start_sim_time": self.start_sim_time
        }
                ...
                if (
                    ...
                    if self.is_vehicle_crossing_line(
                        ...
                        report = {
                            ...
                            "message": message, "start_sim_time": self.start_sim_time
                        }
        ...
        if self.ego_vehicle.is_at_traffic_light():
            ...
            if traffic_light.get_state() == carla.TrafficLightState.Red:
                ...
                if self.on_red:
                    ...
                    self.on_red_speed.append(calculate_velocity(self.ego_vehicle, add_z=True))
            ...
            if self.on_red:
                ...
                if not stopped_at_red:
                    ...
                    r = {
                        ...
                        "tf_id": self.last_red_light.id, "start_sim_time": self.start_sim_time
                    }
                    self.report["report"].append(r)
                    self.last_red_light = None
...
class RunStopSignOracle(BaseOracle):
    ...
    def update(self, time_stamp):
        ...
        if not self._target_stop_sign:
            ...
            if not self.is_actor_affected_by_stop(self.ego_vehicle, self._target_stop_sign):
...
class StuckOracle(BaseOracle):
    ...
    def update(self, time_stamp):
        ...
        if self.stuck_duration > self.STUCK_DURATION:
            r = {"timestamp": time_stamp, "stuck": True, "detected_time": time.time(),
...
class NotReachGoalOracle(BaseOracle):
    ...
    def update(self, time_stamp, goal_loc, waypoint_queue):
        ...
        if dist_to_goal >= self.MIN_DIST:
            ...
            r = {"timestamp": time_stamp, "not_reach_goal": True, "distance": dist_to_goal, "speed": speed,
                 "detected_time": time.time(), "message": message,
                 "start_sim_time": self.start_sim_time}
            self.report["report"].append(r)
...
class SpeedingOracle(BaseOracle):
    ...
    def update(self, time_stamp):
        ...
        if speed > speed_limit and time_stamp > (self.frame_speed_lim_changed + self.T * self.FRAME_RATE):
            message = (
                ...
                f"The timestamp is {time_stamp}. The time detected is {time.time() - self.start_sim_time}")
            r = {"timestamp": time_stamp, "speeding": True, "detected_time": time.time(),
                 "speed": speed, "speed_limit": speed_limit, "message": message, "start_sim_time": self.start_sim_time}
            self.report["report"].append(r)
...
class AllOracle:
    def __init__(self, ego_vehicle, carla_world, report_dir):
        ...
        self.oracles = [
            CollisionOracle(ego_vehicle, carla_world, start_sim_time=self.start_sim_time),
            SpeedingOracle(ego_vehicle, carla_world, start_sim_time=self.start_sim_time),
            LaneInvasionOracle(
            ...
            NotReachGoalOracle(ego_vehicle, carla_world, start_sim_time=self.start_sim_time),
    ...
    def update(self, snapshot, goal_loc, waypoint_queue):
        for oracle in self.oracles:
            if isinstance(oracle, SpeedingOracle):
                ...
                oracle.update(snapshot.timestamp.elapsed_seconds, goal_loc=goal_loc, waypoint_queue=waypoint_queue)
            else:
    ...
    def save_2_file(self):
        ...
        json.dump(self.ego_traj, open(os.path.join(self.report_dir, "ego_traj.json"), "w"), indent=4)
