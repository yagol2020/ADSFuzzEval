#!/bin/bash
if [ -z "$1" ] && [ -z "$CARLA_PORT_DOCKER" ]; then
  echo "请提供端口号作为参数，例如：mysh.sh 12345"
  exit 1
fi
port=${CARLA_PORT_DOCKER:-$1}
carla_name=carla-sim-$port

if [ -z "$2" ]; then
  echo "Open carla display?"
  exit 1
fi
open_carla_display=$2

if [ -z "$DISPLAY" ]; then
  open_carla_display="False"
fi
...
if [ "$open_carla_display" == "True" ]; then
  # 启用显示
  carla_cmd="./CarlaUE4.sh -carla-rpc-port=$port -quality-level=Epic -ResX=800 -ResY=600 && /bin/bash"
