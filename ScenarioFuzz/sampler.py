...
ov_list = ['Ov_type', 'Ov_direction', 'Ov_behavior_v', 'Ov_behavior_w', 'Ov_speed_w', 'Ov_speed_v', 'Ov_name_w',
           'Ov_name_v', 'Ov_R', 'Ov_G', 'Ov_B', 'dp_time']
...
class ValueSampler:
    def __init__(self, value_ranges):
        ...
        self.value_ranges = {key: value_ranges[key] for key in value_ranges.keys() if
                             key not in puddle_list and key not in ov_list}
    ...
    def _sample_random_neighbors(self, num_samples: int = 1, value_range_step: int = 5,
                                 reference_data: Dict[str, Any] = None, value_ranges=None):
        """
        采样方法：随机邻居采样
        """
        ...
        if value_ranges:
            ...
            value_ranges = self.value_ranges
            center_values = {key: center_values[key] for key in center_values.keys() if
                             key not in ['Ov', 'Puddle'] + [k for k in self.ov_ranges.keys()] + [k for k in
                                                                                                 self.puddle_ranges.keys()]}
            ...
            for name, value_range in value_ranges.items():
                if value_range['type'] == ValueType.DISCRETE.value:
                    ...
                    if name not in ['Ov_num', 'Puddle_num']:
                        ...
                        values[name] = random.choice(value_range['values'][
                                                     max(0, v_index - value_range_step):min(len(value_range['values']),
                                                                                            v_index + value_range_step)])
                    elif name == 'Ov_num':
                        ...
                        if values[name] != 0:
                            for index in range(values['Ov_num']):
                                try:
                                    values['Ov'].append(
                                        self._sample_random_neighbors(num_samples=1, value_ranges=self.ov_ranges,
                                                                      reference_data=ov_center_values[index])[0])
                                except BaseException as e1:
                                    print("sampler error", e1)
                    elif name == 'Puddle_num':
                        values[name] = center_values[name]
                        values['Puddle'] = []
                        if values[name] != 0:
                            for index in range(values['Puddle_num']):
                                values['Puddle'].append(
                                    self._sample_random_neighbors(num_samples=1, value_ranges=self.puddle_ranges,
                                                                  reference_data=puddle_center_values[index])[0])
                elif value_range['type'] == ValueType.CONTINUOUS.value:
                    v = center_values[name]
                    values[name] = random_choice(max(v - value_range_step * value_range['step'], value_range['min']),
                                                 min(v + value_range_step * value_range['step'], value_range['max']),
                                                 value_range['step'])
            values_list.append(values)
        return values_list
