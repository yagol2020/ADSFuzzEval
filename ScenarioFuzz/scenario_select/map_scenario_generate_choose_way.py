...
def node_cluster(node_list, dis, debug=False):
    ...
    for i, node_i in enumerate(node_list):
        for j, node_j in enumerate(node_list):
            dist_matrix[i][j] = node2wp(node_i, map).transform.location.distance(
                node2wp(node_j, map).transform.location)
...
def get_route_direction(component, debug=False):
    ...
    if debug:
        ...
        sp_tf = carla.Transform(location=carla.Location(z=50, x=start_loc.x, y=start_loc.y),
...
def main(town):
    ...
    for i, tf in enumerate(traffic_lights_classes_list):
        scenario_dict_tf[i] = {'type': None, 'wp': [], 'wp_mark': [], 'wp_lanetype': [], 'tf_loc': [],
                               'center_loc': None, 'plan_way': []}
    ...
    for t in range(4):
        for i, v in scenario_dict_tf.items():
            new_item = {'type': None, 'wp': [], 'wp_mark': [], 'wp_lanetype': [], 'tf_loc': v['tf_loc'],
                        'center_loc': None, 'plan_way': []}
            for tf_loc in v['tf_loc']:
                tf_n_dis = [node_w.transform.location.distance(carla.Location(*tf_loc)) if node_h[j] == 0 else 9999 for
                            j, node_w in enumerate(node_list_wp)]
                if len(tf_n_dis) == 0:
                    break
    ...
    for ind in range(len(sorted_idx)):
        ...
        if pl_d == 'Straight' and lenth > 30:
            v = {'type': None, 'wp': [], 'wp_mark': [], 'wp_lanetype': [], 'tf_loc': [], 'center_loc': None,
                 'plan_way': []}
