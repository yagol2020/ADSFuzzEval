...
import scenario_select.scontant as S
...
class ScenarioSelector:
    ...
    def get_random_ego_car_start_end_point(self, direction=None, distance=None):
        while True:
            ...
            self.ego_car_wp, self.ego_car_sp, self.ego_car_directin = self.get_wp_sp_direction(wp=wp,
                                                                                               direction=direction,
                                                                                               distance=distance,
                                                                                               choose_rev=True)
            if self.ego_car_wp:
    ...
    def get_random_other_car_start_end_point(self, ov_num):
        ...
        if len(ov_wp_sp_list) != 0:
            for i, o in enumerate(ov_wp_sp_list):
                self.other_w_s_d_list[i] = {'wp': o[0], 'wp_id': self.wp2index(o[0]), 'sp': o[1],
                                            'sp_id': self.wp2index(o[1]), 'direction': o[2]}
    ...
    def add_scenario_to_graph(self, debug=False):
        ...
        for i, node_info in enumerate(self.scenario_node):
            ...
            self.G.add_node(i, pos=to_local_coordinate(self.ego_car_wp, node), type=S.WAYPOINT,
                            mark_type=S.WP_MARK_DICT[node_mark_type], actor_name=S.DEFAULT)
            index_i += 1
        if len(self.scenario['tf_loc']) != 0:
            for i, node in enumerate(self.scenario['tf_loc']):
                self.G.add_node(index_i, pos=to_local_coordinate(self.ego_car_wp, node), type=S.TRAFFIC,
                                mark_type=S.WP_MARK_DICT['Signal_3Light_Post01'], actor_name=S.DEFAULT)
                index_i += 1
    ...
    def G2tensor(self, weather_param):
        ...
        for node in self.G.nodes():
            ...
            node_features.append([node_data['pos'][0], node_data['pos'][1], node_data['pos'][2], node_data['type'],
                                  node_data['mark_type'], node_data['actor_name']])
    ...
    def G2array(self):
        ...
        node_data = np.concatenate(
            [node_pos, node_type.reshape(-1, 1), node_mark_type.reshape(-1, 1), node_actor_name.reshape(-1, 1)], axis=1)
        # 将节点和边属性转换为张量
    ...
    def save_json(self, filepath, score, error, value=None):
        ...
        scenario_data['ego_car'] = {'wp': self.ego_car_wp, 'wp_id': self.ego_car_wp_id, 'sp': self.ego_car_sp,
                                    'sp_id': self.ego_car_sp_id, 'direction': self.ego_car_directin}
