...
from samota.utils import *
from samota.RBF import Model as RBF_Model
from samota.candidate import Candidate
from samota.ensemble import ensemble
...
def get_array_for_crowding_distance(sorted_front):
    ...
    for value in sorted_front:
        ...
        np_array = numpy.array(
            [objective_values[0], objective_values[1], objective_values[2], objective_values[3], objective_values[4],
             objective_values[5]
             ])
...
def GS(database, obj_uncovered, size, g_max, criteria, lb, ub, logger_folder):
    ...
    for i in range(len(T_b)):
        ...
        global_best_uncertainity_log_file = os.path.join(logger_folder,
                                                         'model_global_best_uncertainity_' + str(i) + '.log')
        ...
        for i in range(len(T_b)):
            ...
            with open(global_best_log_file_lst[i], 'a') as f:
                if isinstance(T_b[i], Candidate):
                    f.write('Iteration: ' + str(iteration) + ' fitness: ' + str(
                        T_b[i].get_objective_value(i)) + ' X: ' + str(T_b[i].get_candidate_values()))
                else:
            ...
            with open(global_best_uncertainity_log_file_lst[i], 'a') as f:
                if isinstance(T_n[i], Candidate):
                    f.write('Iteration: ' + str(iteration) + ' uncertainity: ' + str(
                        T_n[i].get_uncertainity_value(i)) + ' X: ' + str(T_b[i].get_candidate_values()))
                else:
                    f.write('Iteration: ' + str(iteration) + ' finded')
                f.write('\n')
    ...
    for i in range(len(T_b)):
        with open(global_best_log_file_lst[i], 'a') as f:
            if isinstance(T_b[i], Candidate):
                f.write('Final: ' + str(iteration) + ' fitness: ' + str(T_b[i].get_objective_value(i)) + ' X: ' + str(
                    T_b[i].get_candidate_values()))
            else:
        ...
        with open(global_best_uncertainity_log_file_lst[i], 'a') as f:
            if isinstance(T_n[i], Candidate):
                f.write('Final: ' + str(iteration) + ' uncertainity: ' + str(
                    T_n[i].get_uncertainity_value(i)) + ' X: ' + str(T_b[i].get_candidate_values()))
            else:
                f.write('Final: ' + str(iteration) + ' finded')
            f.write('\n')
    ...
    for obj in objective_uncovered:
        ...
        for cluster in clusters:
            ...
            with open(os.path.join(logger_folder, 'model_local_best_' + str(obj) + '.log'), 'a') as f:
                f.write('obj: ' + str(obj) + ' cluster_id: ' + str(cluster_id) + ' model_fitness: ' + str(
                    cluster_best_ov) + ' X: ' + str(cluster_best_fv))
                f.write('\n')
    return T_l
...
def run_search(func, size, lb, ub, no_of_Objectives, criteria, archive, database, g_max, store_dir=None):
    ...
    random_population = evaulate_population_with_archive(func, random_population,
                                                         already_executed)  # evaluating whole generation and storing results
    ...
    while (True):
        ...
        with open(sim_global_log_file, 'a') as f:
            ...
            for index in range(len(before_sim)):
                if isinstance(before_sim[index], Candidate):
                    f.write('Item index: ' + str(index) + ' X: ' + str(
                        before_sim[index].get_candidate_values()) + ' Model Fitness: ' + str(
                        before_sim[index].get_objective_values()))
                else:
        ...
        with open(sim_global_log_file, 'a') as f:
            ...
            for index in range(len(T_g)):
                if isinstance(T_g[index], Candidate):
                    f.write('Item index: ' + str(index) + ' X: ' + str(
                        T_g[index].get_candidate_values()) + ' Model Fitness: ' + str(
                        T_g[index].get_objective_values()))
                else:
                    f.write('Item index: ' + str(index) + ' Model Fitness: list')
                f.write('\n')
        ...
        with open(sim_global_log_file, 'a') as f:
            ...
            for index in range(len(T_g)):
                ...
                if isinstance(T_g[index], Candidate):
                    f.write('Item index: ' + str(index) + ' X: ' + str(
                        T_g[index].get_candidate_values()) + ' Simul Fitness: ' + str(
                        T_g[index].get_objective_values()))
                else:
        ...
        with open(sim_local_log_file, 'a') as f:
            ...
            for index in range(len(T_l)):
                ...
                if isinstance(T_l[index], Candidate):
                    f.write('Item index: ' + str(index) + ' X: ' + str(
                        T_l[index].get_candidate_values()) + ' Simul Fitness: ' + str(
                        T_l[index].get_objective_values()))
                else:
                    f.write('Item index: ' + str(index) + ' Simul Fitness: list')
                f.write('\n')
