...
class Config:
    ...
    def check_seed_entropy(self, campaign_cnt, seed):
        town_scenario_prob = {town: {scenario_id: count / campaign_cnt for scenario_id, count in scenario_dict.items()}
                              for town, scenario_dict in self.seed_counter.items()}
        ...
        for seed in seed_scenarios:
            if not seed.startswith(".") and seed.endswith(".json"):
                ...
                with open(seedfile, "r") as fp:
                    seed = json.load(fp)
                seed_info = {'town': seed['town'], 'scenario_id': seed['scenario_id'],
    ...
    def reload_seed_scenarios(self):
        try:
            self.reload_dir_ads = os.path.join(self.reload_dir, self.reload_target.replace(':', '-'),
                                               self.reload_error_type)
